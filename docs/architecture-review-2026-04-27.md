...
- Текущая инфраструктура уже стоит около 12 000 ₽/мес:
  - балансировщик;
  - одна manager-нода кластера;
  - одна worker-нода: 2 CPU / 8 GB RAM / 40 GB boot SSD;
  - 20 GB network SSD.
- Если расширить Kubernetes до отказоустойчивого варианта и добавить хотя бы минимальное количество нод за пределами пилота, ожидаемый cost floor может стать 30 000 ₽+/мес на "типа живом" железе, всё ещё без доказанной готовности к реальным нагрузкам.
...
- фиксированный infra floor: 12 000 ₽/мес сейчас, вероятно 30 000 ₽+/мес при отказоустойчивом пилоте;
...
8. Посчитать себестоимость одного ingest и одного query path:
   - CPU/RAM;
   - текущий infra floor 12 000 ₽/мес;
   - отказоустойчивый floor 30 000 ₽+/мес;
   - Ollama latency;
